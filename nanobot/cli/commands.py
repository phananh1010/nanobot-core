...
def gateway(
    ...
    http_key = (getattr(config.gateway, "http_api_key", None) or "").strip() or None

    try:
        import nanobot.gateway.http  # noqa: F401 — ensure REST stack is present
    except ImportError as e:
        console.print(
            "[red]nanobot.gateway is missing. Install this project from source (e.g. "
            "`uv sync` in the repo) so `nanobot/gateway/http.py` is installed.[/red]"
        )
        raise typer.Exit(1) from e

    console.print(f"{__logo__} Starting nanobot gateway version {__version__} on port {port}...")
    console.print(
        f"[green]✓[/green] HTTP API: http://{gw_host}:{port}/v1/chat "
        f"(GET /health){' [dim](API key required)[/dim]' if http_key else ''}"
    )
    ...
    if channels.enabled_channels:
        ...
        console.print(
            "[yellow]Warning: No chat channels enabled[/yellow] "
            "[dim](Telegram/Slack/Matrix/… — optional; REST /v1/chat above still works.)[/dim]"
        )
